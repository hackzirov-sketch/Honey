...
export function chatRoutes() {
  ...
  router.post("/chats/:id/send/", authRequired, upload.single("file"), (req, res) => {
    ...
    res.status(201).json(chatService.send(String(req.params.id), req.user!.id, data.content, data.message_type, (data as any).file));
  });
  ...
  router.post("/groups/:id/send/", authRequired, upload.single("file"), (req, res) => {
    const data = req.is("multipart/form-data") ? bodyWithFile(req) : sendMessageSchema.parse(req.body);
    res.status(201).json(chatService.groupSend(String(req.params.id), req.user!.id, data.content, data.message_type, (data as any).file));
  });
