...
export const chatRepo = {
  ...
  createMessage(input: { chatId?: string; groupId?: string; senderId: string; content: string; messageType?: string; file?: string | null }) {
    const id = createId("msg");
    const stamp = nowIso();
    sqlite.prepare(`
      INSERT INTO messages (id, chat_id, group_id, sender_id, content, message_type, file, created_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).run(id, input.chatId || null, input.groupId || null, input.senderId, input.content, input.messageType || "text", input.file || null, stamp);
